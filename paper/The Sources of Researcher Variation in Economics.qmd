...
At least in psychology, the variation introduced by researcher choices might outweigh the population variation we typically consider when estimating standard errors [@holzmeister2023heterogeneity]. In finance, @black2024road find that these choices, many of which are unreported, explain heavy variation in estimated effects across 80 different studies on the same policy change.
...
To achieve sufficient power to both show that variation exists *and* explain its sources, our goal was to have at least 90 researchers complete all the steps of the project. 146 research teams finished all tasks, exceeding these requirements.
...
Of those, `r justcount[1, Attrition]` were ineligible for the project. Most ineligible people were graduate students without a forthcoming paper.
...
Appendix @fig-rdd and Table \ref{tab-rdd-reg} show that immediately above the cutoff, completion rates are no different. Local-polynomial regression discontinuity estimates of the effect of the cutoff are insignificant, as are alternate regression discontinuity specifications selected to increase statistical power.[^13] The effect of the cutoff on completion rates is insignificant and positive using a linear slope above the cutoff, or insignificant and negative using a quadratic specification above the cutoff.
...
[^13]: In these alternate versions we use of the full range of the data, rather than a bandwidth, which is justified given that the running variable is randomly assigned aside from the late sign-ups. We also use a zero-order polynomial (average only) below the cutoff, which is justified by the fact that respondents below the cutoff did not know their order. We also find no effect if we drop the late sign-ups from the regression discontinuity analysis.
...
We demonstrate that such variation exists and evaluate explanations for it. We both describe the distribution of effects and researcher choices descriptively, and test our preregistered hypotheses. Preregistered hypotheses that include that the standard deviation of estimated effects, sample sizes, and treated and untreated-group sample sizes will decline from task to task, that peer review will make subsequent effects and sample sizes more similar to the group and to one's peer review partner, and that the standard deviation of reported effects will be bigger than the mean reported standard error. Precise definitions of the  preregistered hypotheses and analyses are shown in Appendix @sec-preregistration.
...
Calculating this ratio across all three tasks shows a decline in the ratio of variance to mean standard error. However, this decline occurs partly because smaller shared samples in later rounds inflate the standard errors (increasing the denominator).
...
The choices shown in Table \ref{tab-estimation-methods} modestly explain estimated effects. Regressing effect sizes on the full set of indicators, which is not preregistered, produces an $R^2$ value of .162 for Task 1, .073 in Task 2, and .023 in Task 3. In Tasks 2 and 3, these choices more heavily influence whether the result is statistically significant at the 95% level, with $R^2$ values of .026 in Task 1, .227 in Task 2, and .338 in Task 3. The choice of estimation method drove most of the variation in significance (and was a significant predictor in Tasks 2 and 3), followed by standard error adjustment (which was significant only in Task 3). 
...
The two comparisons for which an alternative was common enough to compare are for the YRSUSA inclusion and the use of "\< 2007" vs. "\<= 2007" for year of migration, shown in \ref{tab-task1effectsample}. These choices are not associated with large differences in estimated effects. 
