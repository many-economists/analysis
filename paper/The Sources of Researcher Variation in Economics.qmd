...
Appendix @fig-rdd and Table \ref{tab-rdd-reg} show that immediately above the cutoff, completion rates are no different. 
Local-polynomial regression discontinuity estimates of the effect of the cutoff are insignificant, as are alternate regression discontinuity specifications selected to increase statistical power.[^13] 
The effect of the cutoff on completion rates is insignificant and positive using a linear slope above the cutoff, or insignificant and negative using a quadratic specification above the cutoff.
...
[^13]: In these alternate versions we use of the full range of the data, rather than a bandwidth, which is justified given that the running variable is randomly assigned aside from the late sign-ups. 
We also use a zero-order polynomial (average only) below the cutoff, which is justified by the fact that respondents below the cutoff did not know their order. 
We also find no effect if we drop the late sign-ups from the regression discontinuity analysis.
