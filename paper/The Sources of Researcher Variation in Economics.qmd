...
We use a "many-analysts" design, where researchers perform the same task under progressively stricter restrictions on their choices.
...
A "many-analysts" design it to provide multiple teams of researchers with the same data and have them independently try to answer the same research question [@silberzahn2018many].
...
To achieve sufficient power to both show that variation exists *and* explain its sources, our goal was to have at least 90 researchers complete all the steps of the project and, as we discuss in @sec-recruitment-attrition, we had 146 finish all tasks.
...
However, some many-analysts research show that researcher characteristics explain only a small share of the variation [@breznau2021observing]. 
Finally, review may increase agreement if there is an option to revise, although if instead outside evaluation is used as a measure of researcher quality peer review scores do not necessarily predict outlier results [@Menkveld2024; @gould2023same].
...
Like many-analysts studies, the aim of these simulation is to identify how different choices influence results, but they are necessarily limited to decisions identified by the organizers and treat all combinations equally.
One particularly relevant example examines the sensitivity of results in an observational psychological data set to various preprocessing and modeling choices and find significant variation [@klau2023comparing]. 
