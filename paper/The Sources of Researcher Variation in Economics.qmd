...
summary_data <- sumtable(
  ...
  group = 'Round',
  out = 'return' # Return as a data frame instead of LaTeX
) 

# To get rid of Round, remove first row of summary data and remove row labels/names
summary_data <- tail(summary_data, -1)
rownames(summary_data) <- NULL

# Add a custom header above "N," "Mean," and "SD" for each Round group
header <- c(" " = 1, "Original signup" = 3, "Assigned tasks 1" = 3, "Finished task 1" = 3, "Finished task 3" = 3)  


# Render the table with proper spanning headers
summary_data |>
  knitr::kable(
    booktabs = TRUE,
  ...
  add_header_above(header) |> # Adds a header row above the main column names
  kable_styling(
...
```{r Researcher Professional Experience}
